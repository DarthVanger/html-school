...
`,
  ...
  `,
  `
  Нужно вывести то что Внутри Объииекта, а не сам Объиект! Например, мы можем высети <code>x</code> каждой Частицы. В фориче выводи <code>p.x</code> в ДокУмент, а не просто <code>p</code>.
  `,
  `
  ...
  `,
  `
    Поехали Далее. <code>x</code>, <code>y</code> вывели, шо нам дальше надо. ПокумекатьЪ... Позишен Аболют. Позишен Аболют. Поизешн Абсолют &mdash; надо в <code>style</code>. Но Погоди... Стайл не тот что в <code>index.html</code>. А тот <code>style</code>, который для каждой Частицы... Нужно для Каждой Частицы сделать <code>document.querySelector()</code>. А для Квери Селектора нужно <code>id</code>.
  `,
  `
