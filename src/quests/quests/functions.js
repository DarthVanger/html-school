...
export const steps = [
  ...
  {
    task: "Отлично! Функция готова! Осталось ее \"Вызвать\". Под функцией, но до закрывающего &lt;/script&gt;, пишем имя функциии, пустые круглые скобки, и точку с запятой: |c|function xyeta() { ... }\nxyeta();|ce|",
    check: "Прописано xyeta(); после функции, внутри &lt;script&gt;",
    regexp: /}\s+xyeta[(][)];[^<]*<\/script>/,
  },
  {
    task: "Ура:) После запуска должен был текст добавиться в документе.\n Теперь вызовем функцию 3 раза:|c|xyeta();\nxyeta();\nxyeta();",
    check: "Хуета вызвана 3 раза",
    regexp: /}\s+(xyeta[(][)];\s*){3}[^<]*<\/script>/,
  },
  {
    task: "<p>Молодца) Теперь тот же самый текст добавляется к документу 3 раза!</p> <p>В этом смысл функций: мы 1 раз прописали код в функции, и можем его выполнить когда и сколько захотим, вызвав функцию.</p> Вызовы эту хуету 4й раз, и засчитывается :)",
    check: "Хуета вызвана 4 раза",
    regexp: /}\s+(xyeta[(][)];\s*){4}\s*<\/script>/,
  },
];

const skills = ['functions', 'innerHTML'];
